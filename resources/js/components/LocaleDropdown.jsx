    ...
    return (
        <div className={`${classPrefix}-dropdown`} style={{zIndex: 100}}>
            {dropdownLangs.map((lang) => {
                return (<Link href={`${lang.slug}${url}`} className={`flex items-center cursor-pointer no-underline text-[${classPrefix === 'header' ? '#A7DCEB' : (mode === 'positive' || mode === '' ? '#0E0448' : '#B9B1EE')}]`} key={lang.id}>
